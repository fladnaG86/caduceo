...
│  │  Gateway  │←───────│  your-relay.example.com       │←──────│  (PC)  ││
│  │          │        │          │                       │        │        ││
...
Il client si connette in outbound a `wss://your-relay.example.com/ws` e mantiene la connessione aperta con heartbeat.
...
### Linux (systemd)
```bash
caduceo-agent install --relay wss://your-relay.example.com/ws --psk ~/.caduceo/agent.psk
...
```powershell
caduceo-agent install --relay wss://your-relay.example.com/ws --psk %USERPROFILE%\.caduceo\agent.psk
...
- **Cloudflare Tunnel**: il traffico passa da `your-relay.example.com` → Cloudflare → `cloudflared` → `localhost:8443`
