...
:: ══════════════════════════════════════════════════════════════════════════════
:: Caduceo Agent - Self-contained Windows installer
::
:: HOW IT WORKS:
::   1. Locates or extracts the PowerShell script
::   2. Runs it with -ExecutionPolicy Bypass (defeats Restricted/AllSigned/etc.)
::   3. Uses -File instead of -Command (avoids all cmd<->PS escaping hell)
::   4. Handles paths with spaces correctly via quoted -File argument
::
:: SAFE TO DOUBLE-CLICK from File Explorer on Windows 10/11.
:: ══════════════════════════════════════════════════════════════════════════════
...
:: ── Step 1: Find or create the PowerShell script ────────────────────────────

:: Try 1: Same folder as this .bat (most common when both files downloaded together)
set "PS1_FILE=%~dp0install-agent-windows.ps1"
if exist "%PS1_FILE%" goto :found_ps1

:: Try 2: User's Downloads folder (browser default download location)
set "PS1_FILE=%USERPROFILE%\Downloads\install-agent-windows.ps1"
if exist "%PS1_FILE%" goto :found_ps1

:: Try 3: Desktop
set "PS1_FILE=%USERPROFILE%\Desktop\install-agent-windows.ps1"
if exist "%PS1_FILE%" goto :found_ps1

:: Try 4: Same folder as this .bat, but check for case variations
if exist "%~dp0Install-Agent-Windows.ps1" (
    set "PS1_FILE=%~dp0Install-Agent-Windows.ps1"
    goto :found_ps1
)

:: ── No .ps1 found: extract from this .bat file itself ──────────────────────
:: The PowerShell script is embedded below the :PS1_CONTENT marker.
:: We write it to a temp file and run that — this avoids -Command escaping entirely.

echo Script .ps1 non trovato separatamente, estrazione automatica...
echo.

:: Use a temp path that works even if USERPROFILE has spaces
set "PS1_FILE=%TEMP%\caduceo-installer.ps1"

:: Check if PowerShell is available at all
where powershell.exe >nul 2>&1
if %ERRORLEVEL% neq 0 (
    echo ERRORE: PowerShell non trovato! Windows potrebbe essere danneggiato.
    echo.
    pause
    exit /b 1
)

:: Extract the embedded PowerShell content to temp file.
:: We use PowerShell itself to do the extraction (a tiny bootstrap line),
:: since cmd.exe cannot handle multi-line content reliably.
:: The bootstrap reads THIS .bat file, finds the marker, and writes everything
:: after it to the temp .ps1 file. Then we run that .ps1 with -File.
powershell.exe -ExecutionPolicy Bypass -NoProfile -Command "$src=Get-Content -LiteralPath '%~f0' -Raw; $idx=$src.IndexOf([char]10+'::PS1_SCRIPT_START'+[char]10); if($idx -lt 0){$idx=$src.IndexOf([char]13+[char]10+'::PS1_SCRIPT_START'+[char]13+[char]10)}; if($idx -lt 0){Write-Error 'Embedded script not found';exit 1}; $body=$src.Substring($idx); $m=[regex]::Match($body,'::PS1_SCRIPT_START\r?\n'); $body=$body.Substring($m.Index+$m.Length); Set-Content -LiteralPath '%PS1_FILE%' -Value $body -Encoding UTF8"

if %ERRORLEVEL% neq 0 (
    echo ERRORE: Estrazione dello script fallita.
    echo.
    pause
    exit /b 1
)
...
:found_ps1
echo Script trovato: "%PS1_FILE%"
echo.

:: ── Step 2: Run the PowerShell script with -File ────────────────────────────
:: -ExecutionPolicy Bypass: overrides any policy (Restricted, AllSigned, RemoteSigned)
:: -NoProfile: faster startup, avoids user profile side-effects
:: -File: passes the path literally, no variable expansion or escaping issues
::   (unlike -Command, which mangles %USERPROFILE% vs $env:USERPROFILE)

powershell.exe -ExecutionPolicy Bypass -NoProfile -File "%PS1_FILE%"

:: ── Cleanup temp file if we extracted it ─────────────────────────────────────
if exist "%TEMP%\caduceo-installer.ps1" del "%TEMP%\caduceo-installer.ps1" 2>nul
...
:: It is extracted to a temp file and run with -File when no external .ps1
:: is found. This makes the .bat fully self-contained.
:: ══════════════════════════════════════════════════════════════════════════════
::PS1_SCRIPT_START
<#
.SYNOPSIS
